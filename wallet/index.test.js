    ...
    describe('creating a transaction', () => {
        ...
        describe(' and doing the same transaction', () => {
            ...
            it('clones the `sendAmount`output for the transaction ', () => {
                ...
                // filter() will return an array of outputs addressed to recipient. transaction.outputs includes outputs to sender wallet.
                outputsAddressedToRecipient = transaction.outputs.filter(output => output.address === recipient);
                //console.log(outputsAddressedToRecipient)
                // map() will do some processing over each individual item and replace it with something else, here the amount of the output.
                outputAmountsToRecipient = outputsAddressedToRecipient.map(output => output.amount);
                expect(outputAmountsToRecipient).toEqual([sendAmount, sendAmount]);
            });
        });
