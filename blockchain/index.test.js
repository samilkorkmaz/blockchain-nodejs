...
describe("Blockchain", () => {
    ...
    it('adds a new block', () => {
        const validTransactions = "foo";
        blockchain.addBlock(validTransactions);
        expect(blockchain.chain[blockchain.chain.length - 1].validTransactions).toEqual(validTransactions);
    });
    ...
    it('invalidates a chain with a corrupt the genesis block', () => {
        blockchain2.chain[0].validTransactions = 'bad data';
        expect(blockchain.isValidChain(blockchain2.chain)).toBe(false);
    });

    it('invalidates a corrput chain', () => {
        blockchain2.addBlock('foo');
        blockchain2.chain[1].validTransactions = 'not foo';
        expect(blockchain.isValidChain(blockchain2.chain)).toBe(false);
    });
